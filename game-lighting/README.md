...
- **SDK connection works against the real running server**, but requires
  ...
  protocol v4 and sends a plugin-list request on connect; our self-compiled
  OpenRGB 1.0 (see repo root `README.md`) doesn't handle that request and
  the server closes the connection ("recv_select failed receiving magic").
